...
class Pool extends EventEmitter {
  ...
  info () {
    ...
    console.log(`${TAG} using resources:`)
    ls(this._using)
    console.log('\n')

    console.log(`${TAG} idle resources:`)
    ls(this._idle)
    console.log('\n')
  }
  ...
  destroy () {
    const d = (resources) => {
      ...
      let next = it.next()
      while (!next.done) {
        console.log(`${TAG} destroy resource ${next.value.id}`)
        this._destroy(next.value.r)
        next = it.next()
      }
    }
